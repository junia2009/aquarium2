...
export class AnemoneBed {
  constructor(parent, { count = 120 } = {}) {
    ...
    for (let i = 0; i < count * 60 && placed < count; i++) {
      ...
      this.items.push({ x, z, y, size, open: 1 });
      placed++;
    }
...
function urchinGeometry() {
  ...
  // 殻は球ではなく上下に潰れている(高さは直径の半分ほど)。
  //
  // 直す前は、長く平たい三角形の棘を84本まばらに生やしていて、
  // ウニではなくウイルスの模式図に見えていた。効いていたのは
  // 棘の長さではなく密度のほう——短くしてみたら確かにウニに
  // 見えたが、それはバフンウニであってムラサキウニではない。
  // 図鑑に「棘を入れると10cm近い」と書いてある以上、
  // 長さは戻して、密度だけを上げるのが筋
  const TR = 0.42, TH = 0.24;            // 殻。直径0.84 高さ0.48
  const N = 165, LEN = 0.38;             // 棘は殻の直径の 0.45 倍
...
export function createUrchins(parent, count = 40) {
  return new ClingBed(parent, urchinGeometry(), count, 'SPINES', () => {
    ...
    // size は殻の直径ではなく幾何の基準長。殻は 0.84*size、
    // 棘まで入れると 1.60*size になる。図鑑の「殻4〜5cm・棘まで10cm近い」
    // に合わせて 0.045〜0.062 とする(殻 3.8〜5.2cm、全体 7.2〜9.9cm)
    return { x, z, y, size: 0.045 + Math.random() * 0.017, lift: 0,
