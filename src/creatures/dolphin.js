...
export const DOLPHIN_KINDS = {
  ...
  beluga: {
    ...
    length: len(5.00),
    shape: {
      // 3種のなかで飛び抜けて太い。ハクジラでも屈指の胴回りで、
      // 実物の最大胴周は全長の 0.70〜0.85(飼育個体は 0.80 前後)ある。
      // ここを細くすると、いくら額を丸くしても「ずんぐり」に見えない
      hRatio: 0.144, wRatio: 0.126,
      //          吻    メロン →   頂点  首    胸 →  最大 ...................  尾柄
      // 首でくびれた直後に胸が張り、その厚みを体の半ばまで保ってから落とす。
      // 早く細らせると、太さだけ足しても紡錘形に見えてしまう
      hProfile: [0.52, 0.86, 0.94, 0.91, 0.86, 0.97, 1.00, 1.00, 1.00, 0.99, 0.96, 0.91, 0.83, 0.73, 0.61, 0.50],
      wProfile: [0.46, 0.80, 0.88, 0.86, 0.81, 0.94, 1.00, 1.00, 1.00, 0.98, 0.94, 0.88, 0.79, 0.68, 0.55, 0.42],
      // メロンは上へ張り出し、口は下に残る。ここを寝かせると額の丸みが消える。
      // 首の後ろ(t≈0.33)を持ち上げて、うなじの盛り上がりを出す
      yOffset: [0.22, 0.26, 0.27, 0.24, 0.16, 0.08, 0.04, 0.01, 0.00, -0.01, -0.02, -0.02, -0.02, -0.01, 0.0, 0.0],
      ...
      tail: { len: 0.21, height: 0.37, fork: 0.34, horizontal: true },
