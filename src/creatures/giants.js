...
export class HumpbackWhale {
  constructor(scene) {
    const geo = buildFishGeometry({
      // 側面図から背と腹の輪郭を10%刻みで読み取り、そこから逆算した寸法。
      //   体高プロファイル = (背 − 腹) / 2      体軸オフセット = (背 + 腹) / 2
      // 最大体高は全長の 0.237(全長16.5mで3.9m)。
      length: 16.5, height: 1.94, width: 1.42,
      hProfile: [0.15, 0.44, 0.72, 0.91, 0.99, 1.00, 0.97, 0.88, 0.70, 0.45, 0.12],
      wProfile: [0.19, 0.50, 0.78, 0.94, 1.00, 1.00, 0.96, 0.86, 0.66, 0.40, 0.13],
      // 体軸は吻で大きく下がり、体の7〜8割のあたりで最も高くなる。
      // ここがこの種の輪郭を決めている:
      //   ・頭は胴の中心線よりずっと下にぶら下がる(低く平たい吻と、垂れた喉)
      //   ・背は吻から7割地点まで上がり続け、そこが体でいちばん高い
      //     ——「ザトウ(座頭)」「humpback」の名の由来そのもの
      // これを逆(吻で高く)にすると、背が一枚の滑らかなドームになり、
      // 尖った吻を持つイルカの体型になってしまう
      yOffset: [-0.35, -0.36, -0.28, -0.19, -0.09, 0.00, 0.10, 0.22, 0.23, 0.15, -0.06],
      smooth: true,
      rings: 40, radial: 24,
      ...
      tail: { len: 0.22, height: 0.68, fork: 0.40, horizontal: true },
      // 背びれは背のいちばん高いところ(体の7割あたり)に載る小さな鎌
      // 後ろへ大きく反った鎌形。sweep を上げないと四角い旗になる
      dorsal: { from: 0.68, to: 0.81, height: 0.22, sweep: 1.5 },
      // ザトウクジラの象徴、体長3分の1の長い胸びれ。
      // 既定の胸びれは付け根の弦長が0で、根元が一点に集まった刃になる。
      // この種のひれは板状で幅があるので櫂型を使い、
      // 「後ろへ伸びる」のではなく「横へ張り出す」ようにする
      pectoral: {
        shape: 'flipper', at: 0.33,
