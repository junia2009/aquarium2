...
export class Megalodon {
  ...
  constructor(parent, opt) {
    const geo = buildFishGeometry({
      ...
      // ここが「ずんぐりむっくり」の正体だった。
      //
      // height / width は**半値**(断面は yc ± height*profile)。
      // 0.178 と書いていたので体高は 2*0.178*LEN、つまり**全長の
      // 30%**あった。ネズミザメ類の体高は全長の 18% 前後、体幅は
      // 12% 前後で、倍近く太らせていたことになる。
      // 半値で置き直す: 体高 18%/2、体幅 12%/2
      height: LEN * 0.110,
      width: LEN * 0.076,
      // いちばん太いところが前3分の1。
      //
      ...
      // 後ろは「曲線で落とす」より「まっすぐ長く落とす」。参考写真の
      // 後半分は、背びれから尾柄までほぼ直線で細っていく
      hProfile: [0.12, 0.30, 0.53, 0.74, 0.90, 0.99, 1.00, 0.96,
                 0.89, 0.80, 0.70, 0.59, 0.47, 0.35, 0.23, 0.13],
      // 幅は高さより速く落とす。後ろへ行くほど側扁して、
      // 尾柄で「横に薄い板」になる
      wProfile: [0.10, 0.28, 0.52, 0.75, 0.91, 0.99, 1.00, 0.95,
      ...
      yOffset: [0.30, 0.24, 0.16, 0.089, 0.034, 0.003, 0.0, 0.014,
                0.037, 0.068, 0.102, 0.139, 0.180, 0.221, 0.262, 0.296],
      rings: 42, radial: 30,
      ...
      tail: { len: 0.145, height: 0.62, fork: 0.80, lobe: 0.05 },
      dorsal: [
        { from: 0.330, to: 0.500, height: 1.05 },   // 第一背びれ(巨大)
        { from: 0.755, to: 0.815, height: 0.18 },   // 第二背びれ(ごく小さい)
      ],
      anal: { from: 0.785, to: 0.850, height: 0.18 },
      // 胸びれ。len は後ろへの後退量、width は外への張り出しで、
      // どちらも**全長に掛かる**。len 0.40 と書いたら 6.8m 後ろへ
      // 流れる翼になった。ひれの実長はこの2つの合成で、
      // 全長の 20% 前後(=3.4m)に収める
      // 胸びれは大きい。参考写真でも体の深さと同じくらい長い翼が
      // 下へ張り出している。ここが小さいと胴だけが目立って太く見える
      // ひれの実長は len と width の合成。全長の 18% 前後(=4.3m)。
      // 胴を細くしたぶん一度 24% まで伸ばしたら、今度はひれだけが
      // 目立つ「翼のある魚」になった
