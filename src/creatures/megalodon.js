...
const SEGMENTS = [
  [0.05, 0.85], [0.17, 1.48], [0.29, 1.80], [0.41, 1.85],
  [0.53, 1.70], [0.66, 1.40], [0.79, 0.95], [0.91, 0.62],
];
...
export class Megalodon {
  ...
  constructor(parent, opt) {
    const geo = buildFishGeometry({
      ...
      tail: { len: 0.145, height: 0.60, fork: 0.78, lobe: 0.06 },
      dorsal: [
        // 第一背びれ。[1] の全長 10% を保つ——高さの単位は**体高**なので、
        // 体高を上げたぶんこちらを下げる。1.08 * 2.22 = 2.40m ＝ 10.2%
        { from: 0.335, to: 0.505, height: 1.08 },
        { from: 0.755, to: 0.815, height: 0.19 },   // 第二背びれ(ごく小さい)
      ],
      anal: { from: 0.785, to: 0.850, height: 0.19 },
